...
export const API_ROUTE_PREFIX = 'api.innoboxrr.deals.deal_advertiser_agreement_postback.'; // Reemplaza con la ruta adecuada
	...
	return [
		{
			id: 'create',
			name: 'Create',
			callback: 'createDealAdvertiserAgreementPostback',
			icon: 'fa-plus',
			route: true,
			policy: false,
			params: {
...
export const showModel = (modelId, loadRelations = [], loadCounts = [], data = {}) => {
    return makeHttpRequest('get', route(API_ROUTE_PREFIX + 'show'), {
        _token: CSRF_TOKEN,
        deal_advertiser_agreement_postback_id: modelId,
...
export const deleteModel = (data) => {
    ...
    return makeHttpRequest('post', route(API_ROUTE_PREFIX + 'delete'), {
        _token: CSRF_TOKEN,
        _method: 'DELETE',
        deal_advertiser_agreement_postback_id: data.id,
