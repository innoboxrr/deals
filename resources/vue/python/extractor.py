...
def extract_strings(file_path):
    """Extrae cadenas que coincidan con __deals('') de un archivo."""
    with open(file_path, 'r', encoding='utf-8') as file:
        content = file.read()
    matches = re.findall(r"__deals\(['\"](.*?)['\"]\)", content)
    if matches:
        print(f"[INFO] Cadenas encontradas en {file_path}: {matches}")
    return matches

def extract_strings_from_directory(directory):
    """Recorre un directorio y extrae todas las cadenas __deals('') de los archivos."""
    strings = []
