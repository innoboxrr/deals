...
namespace Innoboxrr\Deals\Notifications\DealAdvertiserAgreementCplAdjustment;

use Maatwebsite\Excel\Facades\Excel;
use Innoboxrr\Deals\Exports\DealAdvertiserAgreementCplAdjustmentsExports;
...
class ExportNotification extends Notification
{
    ...
    {
        return config('innoboxrrdeals.notification_via', ['mail', 'database']);
    }
    ...
    {
        Excel::store(
            new DealAdvertiserAgreementCplAdjustmentsExports($this->data), 
            $this->path, 
            config('innoboxrrdeals.export_disk', 's3')
        );
    }

    private function getSubject()
    {
        return env('APP_NAME') . ' | DealAdvertiserAgreementCplAdjustment export notification';
    }
