...
namespace Innoboxrr\Deals\Services\DealAdPlatform\Platforms\Google;

use DateTimeInterface;
use Innoboxrr\Deals\Models\DealAdPlatform;
use Innoboxrr\Deals\Services\DealAdPlatform\Engine\Abstracts\BaseDriver;
use Innoboxrr\Deals\Services\DealAdPlatform\Engine\Contracts\AdsPlatformDriver;
use Innoboxrr\Deals\Services\DealAdPlatform\Platforms\Google\Support\ClientFactory;
use Innoboxrr\Deals\Services\DealAdPlatform\Platforms\Google\Support\Gaql;
use Innoboxrr\Deals\Services\DealAdPlatform\Platforms\Google\Support\Mappers;
...
final class GoogleAdsDriver extends BaseDriver implements AdsPlatformDriver
{
    ...
    {
        $client = $this->gads();
        $cid    = $this->customerId(); // asegúrate que venga SIN guiones (solo dígitos)

        $service = $client->getGoogleAdsServiceClient();

        $request = (new SearchGoogleAdsRequest())
            ->setCustomerId($cid)                         // string de dígitos
            ->setQuery(Gaql::selectCustomerPing());       // p.ej.: "SELECT customer.id, customer.descriptive_name FROM customer LIMIT 1"
    ...
    public function listEntities(string $level, ?string $parentExternalId = null, array $params = []): iterable
    {
        ...
        if ($level === 'campaign') {
            ...
            $query  = Gaql::selectCampaigns(is_string($status) ? $status : null, is_string($search) ? $search : null);
        ...
        if ($level === 'container') {
            ...
            foreach ($stream->iterateAllElements() as $row) {
                ...
                yield [
                    ...
                    'type'        => AdGroupType::name($g->getType()),
                    'metadata'    => ['raw' => json_decode($row->serializeToJsonString(), true)],
                ];
            }
            return;
    ...
    /**
     * Devuelve el customer_id como **string de dígitos** (sin guiones/espacios).
     * Acepta fallback a linked_customer_id si no viene customer_id.
     */
    private function customerId(): string
    {
        $creds = (array) $this->platform->getPayload('credentials', []);

        // 1) toma customer_id o, si no hay, linked_customer_id
        $raw = (string) ($creds['customer_id'] ?? '');
        if ($raw === '') {
            $raw = (string) ($creds['linked_customer_id'] ?? '');
        }
        if ($raw === '') {
            throw new InvalidArgumentException('Falta credentials.customer_id (o linked_customer_id) en el payload.');
        }

        // 2) deja solo dígitos
        $digits = preg_replace('/\D+/', '', $raw) ?? '';

        // 3) valida formato (Google Ads usa 10 dígitos)
        if ($digits === '' || strlen($digits) < 10) {
            throw new InvalidArgumentException("customer_id inválido: '{$raw}' → '{$digits}'");
        }

        return $digits; // p. ej. "1345396760"
    }
    ...
    private function setEntityStatus(string $level, string $externalId, string $status): void
    {
        ...
        if ($level === 'ad') {
            ...
            if ($adGroupId === null) {
                throw new InvalidArgumentException('No se pudo resolver el ad_group asociado al anuncio para cambiar el estado.');
            }
    ...
    private function resolveAdGroupIdForAd(int $adId): ?int
    {
        ...
        $result = $client->getGoogleAdsServiceClient()->search($cid, $query);
